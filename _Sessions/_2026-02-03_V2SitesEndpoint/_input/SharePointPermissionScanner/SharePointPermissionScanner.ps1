...
# 4. Write groups, group members and site users to files
# 5. Run over (site or subsite) and contained subsites, then lists, then list items and write output files

######################### START: VARIABLES TO BE CHANGED BY USER #########################
# LOGGING AND SAVING
$writeEveryXLines = 100
$logEveryXItems = 50
$saveAllJobResultsIntoSingleOutputFiles = $true
...
function newSiteContentsRow($job,$siteUrl,$id,$type,$title,$url){
    $start = if($saveAllJobResultsIntoSingleOutputFiles){"$job,$(CSV-Escape-Text -value $siteUrl),"} else {""}
    return "$($start)$id,$($type),$(CSV-Escape-Text -value $title),$(CSV-Escape-Text -value $url)"
}

$outputFilename2SiteGroups = "02_SiteGroups.csv"
$outputColumns2SiteGroups = @("Id","Role","Title","PermissionLevel","Owner")
function newSiteGroupsRow($job,$siteUrl,$id,$role,$title,$permmissionLevel,$owner){
    $start = if($saveAllJobResultsIntoSingleOutputFiles){"$job,$(CSV-Escape-Text -value $siteUrl),"} else {""}
    return "$($start)$id,$role,$(CSV-Escape-Text -value $title),$(CSV-Escape-Text -value $permmissionLevel),$($owner)"
}

$outputFilename3SiteUsers = "03_SiteUsers.csv"
$outputColumns3SiteUsers = @("Id","LoginName","DisplayName","Email","PermissionLevel","IsGuest","ViaGroup","ViaGroupId","ViaGroupType","AssignmentType","NestingLevel","ParentGroup")
function newSiteUserRow($job,$siteUrl,$id,$loginName,$displayName,$email,$permissionLevel,$isGuest,$viaGroup,$viaGroupId,$viaGroupType,$assignmentType,$nestingLevel,$parentGroup){
    $start = if($saveAllJobResultsIntoSingleOutputFiles){"$job,$(CSV-Escape-Text -value $siteUrl),"} else {""}
    if ($isGuest) { $isGuestText="true" } else { $isGuestText="false" }
    return "$($start)$id,$loginName,$(CSV-Escape-Text -value $displayName),$email,$(CSV-Escape-Text -value $permissionLevel),$isGuestText,$(CSV-Escape-Text -value $viaGroup),$($viaGroupId),$($viaGroupType),$($assignmentType),$($nestingLevel),$(CSV-Escape-Text -value $parentGroup)"
}
...
function newIndividualPermissionItemRow($job,$siteUrl,$id,$type,$title,$url){
    $start = if($saveAllJobResultsIntoSingleOutputFiles){"$job,$(CSV-Escape-Text -value $siteUrl),"} else {""}
    return "$($start)$id,$type,$title,$(CSV-Escape-Text -value $url)"
}

$outputFilename5IndividualPermissionItemAccess = "05_IndividualPermissionItemAccess.csv"
$outputColumns5IndividualPermissionItemAccess = @("Id","Type","Url","LoginName","DisplayName","Email","PermissionLevel","IsGuest","SharedDateTime","SharedByDisplayName","SharedByLoginName","ViaGroup","ViaGroupId","ViaGroupType","AssignmentType","NestingLevel","ParentGroup")
...
if ($saveAllJobResultsIntoSingleOutputFiles) {
    ...
    $outputColumns3SiteUsers = $additionalColumns + $outputColumns3SiteUsers
    $outputColumns4IndividualPermissionItems = $additionalColumns + $outputColumns4IndividualPermissionItems
    $outputColumns5IndividualPermissionItemAccess = $additionalColumns + $outputColumns5IndividualPermissionItemAccess
...
function isGuestAccount($loginName){ return ([string]$loginName).Contains("#ext#") }
...
function addGroupAndGroupMembersToOutputLines() {
    param(
        ...
        [Parameter(Mandatory=$true)][AllowEmptyCollection()][System.Collections.ArrayList]$groupOutputLines,
        [Parameter(Mandatory=$true)][AllowEmptyCollection()][System.Collections.ArrayList]$userOutputLines,
    ...
    $groupOutputLines.Add( (newSiteGroupsRow -job ($jobIndex+1) -siteUrl $siteUrl -id $group.Id -role $role -title $group.Title -permmissionLevel $permissionLevel -owner "") ) | Out-Null
    ...
    foreach($m in $groupMembers){
        ...
        $userOutputLines.Add( (newSiteUserRow -job ($jobIndex+1) -siteUrl $siteUrl -id $id -loginName $loginName -displayName $m["DisplayName"] -email $m["Email"] -permissionLevel $permissionLevel -isGuest $isGuest -viaGroup $m["ViaGroup"] -viaGroupId $m["ViaGroupId"] -viaGroupType $m["ViaGroupType"] -assignmentType "Group" -nestingLevel $m["NestingLevel"] -parentGroup $m["ParentGroup"]) ) | out-null
        if($id -ne "") { $sitePrincipalIdsThatHaveBeenWrittenToFile[$id] = "" }
        if($loginName -ne "") { $sitePrincipalIdsThatHaveBeenWrittenToFile[$loginName] = "" }
    }
}
    ...
    foreach($member in $members){
        ...
        switch ($member.PrincipalType){
            ...
            "SecurityGroup" {
                ...
                $groupId = [string]($member.LoginName.Split("|")[-1])
                # if groupId not available or invalid or group should not be resolved, add as user and continue
                if ( ($groupId -eq "") -or $groupId.Contains("/") -or $groupId.Contains("\") -or $groupId.Contains("@") -or $doNotResolveTheseGroups.Contains($member.Title) ) {
                    $groupMembers.Add( @{"Id"=$member.Id; "LoginName"=$loginName; "DisplayName"=$member.Title; "Email"=$member.Email; "ViaGroup" = $group.Title; "ViaGroupId" = $group.Id; "ViaGroupType"="SharePointGroup"; "NestingLevel"=1; "ParentGroup"=""} ) | out-null
                    continue
                }                
...
function getAzureGroupMembers(){
    ...
    if ($recurse){
        ...
        while ( ($groupStack.Count -gt 0) -and ( $nestingLevel -le $maxGroupNestinglevel) ) {
            ...
            $isAccount = ( $viaGroupId.Contains("@") -or $viaGroupId.Contains("\") -or $viaGroupId.Contains("/") -or $doNotResolveTheseGroups.Contains($viaGroup) )
            if ($isAccount) {
                # add group as user
                $loginName = $groupMember.UserPrincipalName
                $groupMembers.Add( @{"LoginName" = $viaGroupId; "DisplayName" = $viaGroup; "Email" = ""; "ViaGroup" = ""; "ViaGroupId" = ""; "ViaGroupType" = ""; "NestingLevel" = $groupNestingLevel+1; "ParentGroup" = $parentGroupName} ) | Out-Null
                
            } else {
                if ( $useAzureActiveDirectoryPowerShellModule){
                    try { $members = Get-AzureADGroupMember -ObjectId $viaGroupId -All $true }
                    catch {
                        Write-Host "ERROR: Get-AzureADGroup -ObjectId $viaGroupId" -f white -b red; Write-Host $_
                    }
                } else {
...
# ----------------- START: Step 3 - Detect $urlType [Site|Subsite|Library|Folder|Error], $siteUrl and $libraryName -----------------
...
# ----------------- END: Step 3 - Detect $urlType [Site|Subsite|Library|Folder|Error], $siteUrl and $libraryName -----------------
    ...
    if(!$saveAllJobResultsIntoSingleOutputFiles -or $jobIndex -eq 0){ $outputLines1SiteContents.Add(($outputColumns1SiteContents -join ",")) | Out-Null }
    ...
    if(!$saveAllJobResultsIntoSingleOutputFiles -or $jobIndex -eq 0){ $outputLines2SiteGroups.Add(($outputColumns2SiteGroups -join ",")) | Out-Null }
    $outputFile2SiteGroups = Join-Path $outputFolder $outputFilename2SiteGroups

    $outputLines3SiteUsers = [System.Collections.ArrayList]@()
    if(!$saveAllJobResultsIntoSingleOutputFiles -or $jobIndex -eq 0){ $outputLines3SiteUsers.Add(($outputColumns3SiteUsers -join ",")) | Out-Null }
    ...
    if(!$saveAllJobResultsIntoSingleOutputFiles -or $jobIndex -eq 0){ $outputLines4IndividualPermissionItems.Add(($outputColumns4IndividualPermissionItems -join ",")) | Out-Null }
    ...
    if(!$saveAllJobResultsIntoSingleOutputFiles -or $jobIndex -eq 0){ $outputLines5IndividualPermissionItemAccess.Add(($outputColumns5IndividualPermissionItemAccess -join ",")) | Out-Null }
...
# ------------------ START: Step 4 - Write groups, group members and site users to files ------------------
    ...
    foreach ( $group in $groups ) {
        if ($doNotResolveTheseGroups.Contains($group.Title)) { continue }
    ...
    foreach ($roleAssignment in $roleAssignments) {
        if ($roleAssignment.Member.PrincipalType -eq "User") {
            ...
            if (!$sitePrincipalIdsThatHaveBeenWrittenToFile.Contains($loginName)) {
                ...
                if ($permissionLevels.Count -gt 0){            
                    $outputLines3SiteUsers.Add( (newSiteUserRow -job ($jobIndex+1) -siteUrl $siteUrl -id $id -loginName $loginName -displayName $displayName -email $email -permissionLevel $permissionLevel -isGuest $isGuest -viaGroup "" -viaGroupId "" -viaGroupType "" -assignmentType "User" -nestingLevel 0 -parentGroup "") ) | Out-Null
...
# ------------------ START: Step 5 - Run over (site or subsite) and contained subsites and write output files ------------------
    ...
    for ($siteIndex=0; $siteIndex -lt $siteCount; $siteIndex++) {
        ...
        if ($isSubSite -eq $true){
            ...
            $outputLines1SiteContents.Add( (newSiteContentsRow -job ($jobIndex+1) -siteUrl $siteUrl -id $web.Id -type "Subsite" -title $web.Title -url $web.Url) ) | Out-Null
            # if subsite has broken permission inheritance, add to individual permission items file
            if ($web.HasUniqueRoleAssignments){ $outputLines4IndividualPermissionItems.Add( (newIndividualPermissionItemRow -job ($jobIndex+1) -siteUrl $siteUrl -id $web.Id -type "Subsite" -title $web.Title -url $currentSiteUrl) ) | Out-Null }
        }
        ...
        foreach ($list in $lists) {
            ...
            $outputLines1SiteContents.Add( (newSiteContentsRow -job ($jobIndex+1) -siteUrl $siteUrl -id $list.Id -type $listType -title $list.Title -url $listUrl) ) | Out-Null
            # if list has broken permission inheritance, add to individual permission items file
            if ($list.HasUniqueRoleAssignments){ $outputLines4IndividualPermissionItems.Add( (newIndividualPermissionItemRow -job ($jobIndex+1) -siteUrl $siteUrl -id $list.Id -type $listType -title $web.Title -url $listUrl ) ) | Out-Null }
            ...
            for ($i=0; $i -lt $allItems.Count; $i++) {
                ...
                if ($true) {
                    ...
                    $outputLines4IndividualPermissionItems.Add( (newIndividualPermissionItemRow -job ($jobIndex+1) -siteUrl $siteUrl -id $item.Id -type $itemType -title $itemTitle -url $itemUrl ) ) | Out-Null
                    ...
                    if ( ($item.FieldValues["SharedWithDetails"] -ne $null) -and (([string]$item.FieldValues["SharedWithDetails"]).Length -gt 0) ){
                        ...
                        $sharedWithDetailsObject.PSObject.Properties | ForEach-Object {
                            ...
                            if ($global:userDisplayNameCache.Contains($sharedByLoginName)) { $sharedByDisplayName = $global:userDisplayNameCache[$sharedByLoginName] } else { $sharedByDisplayName = "" }
                        ...
                        switch ($principalType) {
                            ...
                            { $_ -in "SharePointGroup", "SecurityGroup" } {
                                if ($principalType -eq "SharePointGroup") {
                                    if ($omitSharePointGroupsInBrokenPermissionsFile){ continue }
                                    $principalId = $roleAssignment.Member.Id
                                    $principalTitle = $roleAssignment.Member.Title
                                    $group = $global:sharePointGroupCache[$principalId]
                                    ...
                                    # if group has not yet been written to site groups, write group to file
                                    if ( ($groupMembers.Count -gt 0) -and (!$sitePrincipalIdsThatHaveBeenWrittenToFile.ContainsKey($principalId)) ){
                                        $outputLines2SiteGroups.Add( (newSiteGroupsRow -job ($jobIndex+1) -siteUrl $siteUrl -id $principalId -role "" -title $principalTitle -permmissionLevel "[Individual items]" -owner "" ) ) | out-null
                                        $sitePrincipalIdsThatHaveBeenWrittenToFile[$principalId] = ""
                                    }
                                } else {
                                    # M365 group example: $member.LoginName = 'c:0o.c|federateddirectoryclaimprovider|0319638f-ac31-4d7a-bfbf-7658f770c619_o'
                                    $groupId = [string]($roleAssignment.Member.LoginName.Split("|")[-1])
                                    $displayName = [string]($roleAssignment.Member.Title)
                                    # if not groupId not available, add as user and continue
                                    if ( ($groupId -eq "") -or $groupId.Contains("/") -or $groupId.Contains("\") -or $groupId.Contains("@") -or $doNotResolveTheseGroups.Contains($displayName) ){
                ...
                } # if item has unique role assignments
