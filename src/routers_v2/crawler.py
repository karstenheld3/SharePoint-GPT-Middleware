...
async def _selftest_stream(storage_path: str, skip_cleanup: bool, max_phase: int, logger: MiddlewareLogger, openai_client, base_url: str, crawler_cfg: dict):
  ...
  try:
    ...
    if max_phase >= 15:
      ...
      # K5: VS_DELETED (C4) - verify vector store recreation when deleted
      yield next_test("K5: Vector store recreation when deleted (C4)")
      if openai_client:
        try:
          # Get current vector_store_id from domain
          domain = load_domain(storage_path, SELFTEST_DOMAIN_ID, logger)
          old_vs_id = domain.vector_store_id
          if old_vs_id:
            # Delete the vector store from OpenAI
            yield log(f"  Deleting vector store '{old_vs_id}'...")
            try:
              await openai_client.vector_stores.delete(old_vs_id)
              yield log("  Vector store deleted.")
            except Exception as e:
              yield log(f"  Delete failed (may already be deleted): {str(e)}")
            
            # Run incremental crawl - should detect missing VS and recreate
            yield log("  Running incremental crawl...")
            result = await run_crawl_and_track(SELFTEST_DOMAIN_ID, "crawl", mode="incremental", scope="files", source_id="files_all")
            
            # Reload domain and verify new vector_store_id
            domain_after = load_domain(storage_path, SELFTEST_DOMAIN_ID, logger)
            new_vs_id = domain_after.vector_store_id
            
            if new_vs_id and new_vs_id != old_vs_id:
              yield check_ok(f"New VS created: {old_vs_id[:20]}... -> {new_vs_id[:20]}...")
            elif new_vs_id == old_vs_id:
              yield check_fail(f"VS ID unchanged (should have been recreated)")
            else:
              yield check_fail("No vector_store_id in domain after crawl")
          else:
            yield check_ok("Skipped (no vector_store_id in domain)")
        except Exception as e:
          yield check_fail(f"Test failed -> {str(e)}")
      else:
        yield check_ok("Skipped (OpenAI not configured)")
