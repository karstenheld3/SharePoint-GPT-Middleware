...
# --- Streaming operation state management ---

def get_streaming_state_folder(persistent_storage_path: str) -> str:
  """Get the state folder path for streaming operations, creating it if needed."""
  if not persistent_storage_path:
    return None
  state_folder = os.path.join(persistent_storage_path, "state")
...
def get_streaming_current_state(state_folder: str, operation_id: str) -> str:
  ...
  if streaming_state_file_exists(state_folder, operation_id, "running"):
    return "running"
  if streaming_state_file_exists(state_folder, operation_id, "paused"):
    return "paused"
  return None
...
def list_streaming_operations(state_folder: str, endpoint_filter: str = None) -> List[Dict[str, str]]:
  ...
  for state in ["running", "paused"]:
    ...
    for file_path in glob.glob(pattern):
      ...
      if endpoint_filter and not operation_id.startswith(endpoint_filter):
        continue
