...
def create_logfile(logfile_path: str, initial_content: str, log_data: Dict[str, Any] = None) -> str:
  """
  Create a new logfile with initial content (overwrites if exists).
  
  Args:
    logfile_path: Full path to the logfile
    initial_content: Initial content to write to the file
    log_data: Optional logging context from log_function_header
  ...
  try:
    ...
    parent_dir = os.path.dirname(logfile_path)
    if parent_dir and not os.path.exists(parent_dir):
      os.makedirs(parent_dir)
      if log_data:
        log_function_output(log_data, f"Created directory: {parent_dir}")
    
    # Write the logfile (overwrite if exists)
    with open(logfile_path, 'w', encoding='utf-8') as f:
      f.write(initial_content)
    
    if log_data:
      log_function_output(log_data, f"Created logfile: {logfile_path}")
    
    return initial_content
  except Exception as e:
    error_msg = f"Failed to create logfile {logfile_path}: {str(e)}"
    if log_data:
      log_function_output(log_data, f"ERROR: {error_msg}")
    raise


def append_to_logfile(logfile_path: str, content: str, log_data: Dict[str, Any] = None) -> None:
  """
  Append content to an existing logfile.
  ...
  try:
    ...
    if log_data:
      log_function_output(log_data, f"Appended to logfile: {logfile_path}")
  except Exception as e:
    error_msg = f"Failed to append to logfile {logfile_path}: {str(e)}"
    if log_data:
      log_function_output(log_data, f"ERROR: {error_msg}")
    raise