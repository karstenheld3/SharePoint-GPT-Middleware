...
def get_job_metadata_from_file_path(persistent_storage_path: str, file_path: str) -> Optional[JobMetadata]:
  try:
    ...
    monitor_url = (start_data or {}).get("monitor_url", f"/v2b/jobs/monitor?job_id={job_id_in_file}&format=stream")
...
class StreamingJobWriter:
  ...
  def __init__(self, persistent_storage_path: str, router_name: str, action: str, object_id: Optional[str], source_url: str, router_prefix: str = "/v2b", buffer_size: int = CRAWLER_HARDCODED_CONFIG.PERSISTENT_STORAGE_LOG_EVENTS_PER_WRITE):
